...
#2nd -------------> provisioning web with bootstrap file perform web ansible tasks
resource "null_resource" "web" {
  ...
  provisioner "remote-exec" {
    inline = [ 
        ...
        "sudo sh /tmp/bootstrap.sh web dev"  #we r passing component and env values here

     ]
  }
}

# #3rd-------------------->to stop the instance only after running web null resource
...
resource "null_resource" "web_delete" {
  ...
  command = "aws ec2 terminate-instances --instance-ids ${module.web.id} --region us-east-1"
  }
