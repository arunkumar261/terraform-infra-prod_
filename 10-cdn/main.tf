resource "aws_cloudfront_distribution" "arundev" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.arundev.domain_name
        zone_id = aws_cloudfront_distribution.arundev.hosted_zone_id
      }
    }
  ]
}